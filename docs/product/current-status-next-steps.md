...
- `feature/requests-scheduling-admin-parity` adds consumer request history, showing appointments, admin dashboard/users/showings, stricter role scoping, web/mobile listing-detail parity, a collapsible admin sidebar, role-filtered user management, and a safe Resend/ClickSend notification foundation with compose/resend actions, scheduled-showing notifications, Guam phone normalization, and guarded consumer-only initial request notifications.
