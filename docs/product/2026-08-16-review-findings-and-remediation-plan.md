...
- Routing/tenancy: automated coverage includes browser-host precedence, conflicting native slugs, forged referer rejection, unknown/inactive tenant rejection, cross-broker profiles, intent, leads, tasks, and showings, platform-admin access, rollback safety, broker-domain notification links, protection against deleting an active brokerage's last active domain, and controlled handling of concurrent primary-domain conflicts. Native API requests now reassert their build-time brokerage slug after caller headers are applied.
